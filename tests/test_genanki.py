...
TEST_CN_MODEL = genanki.Model(
    345678, 'Chinese',
    fields=[{'name': 'Traditional'}, {
        'name': 'Simplified'}, {'name': 'English'}],
    templates=[
        {
            'name': 'Traditional',
            'qfmt': '{{Traditional}}',
...
class TestWithCollection:
    ...
    def test_multi_deck_package(self):
        ...
        all_imported_decks = self.col.decks.all()
        # default deck, foodeck, and bardeck
        assert len(all_imported_decks) == 3
    ...
    def test_notes_generate_cards_based_on_req__with_hint(self):
        ...
        n1 = genanki.Note(model=TEST_MODEL_WITH_HINT, fields=[
                          'capital of California', '', 'Sacramento'])
        n2 = genanki.Note(model=TEST_MODEL_WITH_HINT, fields=[
    ...
    def test_media_files(self):
        ...
        self.import_package(genanki.Package(
            deck, media_files=['present.mp3', 'present.jpg']))
    ...
    def test_media_files_absolute_paths(self):
        ...
        self.import_package(genanki.Package(deck, media_files=[
                            present_mp3_path, present_jpg_path]))
    ...
    def test_deck_with_description(self):
        deck = genanki.Deck(
            112233, 'foodeck', description='This is my great deck.\nIt is so so great.')
        note = genanki.Note(TEST_MODEL, ['a', 'b'])
        deck.add_note(note)
    ...
    def test_card_added_date_is_recent(self):
        ...
        # Jan 1 2020 UTC (milliseconds since epoch)
        assert anki_card.id > 1577836800000
